...
describe('basic', () => {
  const vec1 = [5, 23, 2, 5, 9]
  const vec2 = [3, 21, 2, 5, 14]

  const fvec1 = new Float64Array(vec1)
  const fvec2 = new Float64Array(vec2)

  bench('@moeru-ai/cosine-similarity', () => {
    moeruAICosineSimilarity(fvec1, fvec2)
  })

  bench('ai', () => {
    aiCosineSimilarity(vec1, vec2)
  })
